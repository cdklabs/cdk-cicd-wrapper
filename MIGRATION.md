...
> adopt v3 on a branch while 0.x stays in production. Some Blueprint capabilities are not in v3 yet — those rows
...
The **logical IDs inside the stack are unchanged** between Blueprint and v3, so once the names match it is a
clean in-place update, not a resource replacement. To match Blueprint's name, use `stageStackName` (a v3
TS-authoring helper) in your `bin/`:

```ts
import { stageStackName } from '@cdklabs/cdk-cicd-wrapper';

// Reproduces Blueprint's `DEV-myapp` / `PROD-myapp`, so v3 UPDATES the existing stack in place.
new MyStack(app, 'myapp', { stackName: stageStackName('myapp', { stageFirst: true, uppercaseStage: true }) });
```

**`uppercaseStage` matches Blueprint's *default* stages only.** cdk prefixed the stack name with your stage id
...
resources behind, un-owned), then adopt those resources into the v3 stack with `cdk import`. Note the Blueprint
