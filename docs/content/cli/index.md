...
| `synth-ci` | Synthesize the pipeline app to a cloud assembly **without deploying** — the safe pre-flight for `deploy-ci`. Renders exactly what `deploy-ci` would provision (same engine routing) into `--output` (default `cdk.out`) so you can inspect or `cdk diff` it first. |
| `list-ci` | List the stacks (and, with `--resources`, a per-stack resource-type breakdown) of the pipeline app `deploy-ci` would provision — a quick inventory that leaves no `cdk.out` behind. |
| `deploy-ci` | Provision the pipeline itself into the hub account, from `cicd.config.ts` alone (the one command a user runs by hand; everything after it is the pipeline deploying the application). `--disposable` deletes the pipeline's artifact bucket and key together with the stack (for throwaway pipelines). Preview it first with `synth-ci` / `list-ci`. |
