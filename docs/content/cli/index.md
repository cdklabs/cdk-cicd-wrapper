...
> **Experimental.** The zero-touch pipeline commands below are pre-release and still evolving; flags and
...
## zero-touch pipeline (experimental)
...
| `migrate` | Generate a zero-touch `cicd.config.ts` from an existing 0.4.x `PipelineBlueprint` entry file (`--entry`, `--application`, `--dry-run`). |
