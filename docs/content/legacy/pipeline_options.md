...
!!! warning "Blueprint (0.x) only — dropped in zero-touch"
    ...
    exposes only `config`, `stages`, and `pipelineName` — none of these four knobs exist in zero-touch's
    `CdkPipelinesEngine`, and there is no other config surface (`cicd.config.ts`) that reaches them.
    They are dropped, not deferred; there is no zero-touch equivalent today.
