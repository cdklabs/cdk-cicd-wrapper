...
async def check_ci_configuration(
    project_path: str,
    ctx: Context
) -> Dict:
    ...
    try:
        ...
        if codepipeline_in_code and github_actions_in_code:
            results["issues"].append("Both CodePipeline and GitHub Actions configurations found")
            results["recommendations"].append("Use only one CI system - either CodePipeline or GitHub Actions")
            ...
            if not codebuild_image:
                results["recommendations"].append("Consider specifying a CodeBuild image with .codeBuildImage()")
            
            if not codebuild_version:
                results["recommendations"].append("Consider specifying a CodeBuild version with .codeBuildVersion()")
            
            # Check if Docker image is outdated
            if codebuild_image and "aws/codebuild/amazonlinux2-x86_64-standard:" in codebuild_image:
                image_version = codebuild_image.split(":")[-1] if ":" in codebuild_image else ""
                try:
                    # Simple version check - recommend newer if it seems outdated
                    if image_version and float(image_version) < 3.0:
                        results["recommendations"].append(f"Consider updating CodeBuild image to a newer version (current: {image_version})")
            ...
            for workflow_file in github_actions_files:
                try:
                    with open(workflow_file, 'r') as f:
                        ...
                        if "build" in workflow_content and ("npm" in workflow_content or "yarn" in workflow_content):
                            build_workflow_found = True
                        if "deploy" in workflow_content or "cdk deploy" in workflow_content:
                            deploy_workflow_found = True
                except Exception:
                    pass
            
            if not build_workflow_found:
                results["recommendations"].append("Consider adding a GitHub Actions workflow for building the project")
            
            if not deploy_workflow_found:
                results["recommendations"].append("Consider adding a GitHub Actions workflow for deploying with CDK")
            
        else:
            results["ci_system"]["type"] = "none"
            results["issues"].append("No CI system configuration found")
            results["recommendations"].append("Configure either CodePipeline or GitHub Actions for CI/CD")
        ...
        results["ci_system"]["environment_variables"] = {k: v for k, v in ci_env_vars.items() if v}
        
        # Warn about skipping important steps
        if all_env_vars.get("SKIP_VALIDATION") == "true":
            results["issues"].append("Validation is being skipped (SKIP_VALIDATION=true)")
            results["recommendations"].append("Only skip validation temporarily for development purposes")
        
        if all_env_vars.get("SKIP_TESTS") == "true":
            results["issues"].append("Tests are being skipped (SKIP_TESTS=true)")
            results["recommendations"].append("Only skip tests temporarily for development purposes")
