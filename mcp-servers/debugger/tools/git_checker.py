    ...
    try:
        ...
        for file_path in entry_files:
            try:
                ...
                if re.search(r'\.codecommit\(', content) or "CodeCommitRepository" in content:
        ...
        codecommit_configured = codecommit_in_code or package_json_repo_type == "CODECOMMIT"
        ...
        if github_configured and codecommit_configured:
            ...
            results["recommendations"].append("Use only one Git provider - either GitHub or CodeCommit")
            ...
            if package_json_repo_type == "GITHUB":
                ...
                if not github_vars["GITHUB_CONNECTION_ARN"]:
                    results["issues"].append("GitHub repository type is configured but GITHUB_CONNECTION_ARN is missing")
                    results["recommendations"].append("Set GITHUB_CONNECTION_ARN environment variable for GitHub integration")
                    results["valid"] = False
                else:
                    ...
                    if not connection_arn.startswith("arn:aws:codestar-connections:"):
                        results["issues"].append("GITHUB_CONNECTION_ARN does not appear to be a valid CodeStar connection ARN")
                        results["recommendations"].append("Verify GITHUB_CONNECTION_ARN format - should start with arn:aws:codestar-connections:")
            elif github_in_code:
                results["recommendations"].append("GitHub configuration found in code but repository type not set to GITHUB in package.json")
            ...
            if all_env_vars.get("ACCOUNT_RES") and all_env_vars.get("RES_ACCOUNT_AWS_PROFILE"):
                ...
                try:
                    ...
                    if "config" in package_data and "repositoryName" in package_data["config"]:
                    ...
                    results["provider"]["connectivity_test"] = {
                        ...
                        "specific_repository_exists": repo_exists if repo_name else None
                    }
                    
                    # Check if there are any repositories in the account
                    if repositories_count == 0:
                        results["issues"].append("No CodeCommit repositories found in the RES account")
                        results["recommendations"].append("CodeCommit is an end-of-life service with restrictions. Create at least one repository in the RES account for valid configuration")
                        results["valid"] = False
                    
                    # Only add a recommendation for a specific repo if the account has repositories
                    elif repo_name and not repo_exists:
                        results["recommendations"].append(f"CodeCommit repository '{repo_name}' does not exist in the RES account. You can create it or use one of the {repositories_count} existing repositories")
                    ...
                    results["issues"].append(f"CodeCommit connectivity test failed: {str(e)}")
                    results["recommendations"].append("Check AWS credentials and CodeCommit permissions")
            else:
                results["issues"].append("CodeCommit is configured but ACCOUNT_RES or RES_ACCOUNT_AWS_PROFILE is missing")
                results["recommendations"].append("Set ACCOUNT_RES and RES_ACCOUNT_AWS_PROFILE for CodeCommit access validation")
        else:
            results["provider"]["type"] = "none"
            results["issues"].append("No Git provider configuration found")
            results["recommendations"].append("Configure either GitHub or CodeCommit as your Git provider")
        
        # Check for configuration inconsistency
        if (config_sources["github"]["environment_vars"] or config_sources["github"]["in_code"]) and package_json_repo_type == "CODECOMMIT":
            results["issues"].append("Inconsistent configuration: GitHub settings found in environment/code but package.json specifies CODECOMMIT")
            results["recommendations"].append("Align your package.json config.repositoryType with your environment variables and code")
        
        if config_sources["codecommit"]["in_code"] and package_json_repo_type == "GITHUB":
            results["issues"].append("Inconsistent configuration: CodeCommit settings found in code but package.json specifies GITHUB")
            results["recommendations"].append("Align your package.json config.repositoryType with your code")
