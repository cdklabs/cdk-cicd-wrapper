...
async def check_plugins(
    ...
    try:
        ...
        plugin_keywords = [
            "plugin", "addPlugin", "registerPlugin", "createPlugin", 
            "PipelinePlugin", "CDKPlugin", "IPlugin"
        ]
        
        # Security-sensitive patterns to look for
        security_patterns = {
            "public_access": [
                r"publicRead(able)?", r"publicAccess", r"AllowPublic", r"makePublic", 
                r"publiclyAccessible\s*[=:]\s*true", r"publicDns", r"enablePublic"
            ],
            ...
            "sensitive_data_handling": [
                r"noEncryption", r"disableEncryption", r"skipEncryption",
                r"encryption\s*[=:]\s*(?:false|undefined|null)",
                r"enableKeyRotation\s*[=:]\s*false"
            ],
            "insecure_defaults": [
                r"requireSsl\s*[=:]\s*false", r"insecure\s*[=:]\s*true", 
                r"skipValidation", r"bypassApproval", r"bypassSecurityCheck"
            ]
        }
            ...
            try:
                ...
                for keyword in plugin_keywords:
                    if keyword in content:
                        ...
                        class_matches = re.findall(r'class\s+([A-Za-z0-9_]+Plugin)\b', content)
                        plugin_classes.extend(class_matches)
                        
                        var_matches = re.findall(r'const\s+([A-Za-z0-9_]+Plugin)\s*=', content)
                        ...
                        if plugin_classes:
                            ...
                            usage_match = re.search(r'\.addPlugin\(\s*(?:new\s+)?([A-Za-z0-9_]+)', content)
                            if usage_match:
                                plugin_name = usage_match.group(1)
                        
                        # Skip if this looks like a standard import or reference
                        if plugin_name and not any(plugin["name"] == plugin_name for plugin in detected_plugins):
        ...
        if security_concerns:
            for concern in security_concerns:
                if concern["type"] == "public_access":
                    results["issues"].append(f"Plugin '{concern['plugin']}' in {concern['file']} may enable public access")
                    results["recommendations"].append(f"Review {concern['plugin']} for public access settings that might expose data")
                
                elif concern["type"] == "broad_iam_permissions":
                    results["issues"].append(f"Plugin '{concern['plugin']}' in {concern['file']} may use overly permissive IAM policies")
                    results["recommendations"].append(f"Review {concern['plugin']} to ensure it follows least privilege principle")
                
                elif concern["type"] == "sensitive_data_handling":
                    results["issues"].append(f"Plugin '{concern['plugin']}' in {concern['file']} may handle sensitive data insecurely")
                    results["recommendations"].append(f"Ensure {concern['plugin']} uses encryption for sensitive data")
                
                elif concern["type"] == "insecure_defaults":
                    results["issues"].append(f"Plugin '{concern['plugin']}' in {concern['file']} may use insecure defaults")
                    results["recommendations"].append(f"Review default security settings in {concern['plugin']}")
        
        # General recommendations for plugin usage
        if detected_plugins:
            results["recommendations"].append("Always review custom plugins for security implications before deployment")
            results["recommendations"].append("Consider adding plugin-specific tests to validate behavior")
