...
async def check_vpc_configuration(project_path: str, ctx: Context) -> Dict:
    ...
    try:
        ...
        if (
            vpc_env_vars["PROXY_SECRET_ARN"]
            or vpc_env_vars["HTTP_PROXY"]
            or vpc_env_vars["HTTPS_PROXY"]
        ):
            vpc_proxy_configured = True
        ...
        if vpc_proxy_configured:
            ...
            if missing_vars:
                results["issues"].append(
                    f"VPC proxy is configured but missing required environment variables: {', '.join(missing_vars)}"
                )
                results["recommendations"].append(
                    "Set all required proxy environment variables for VPC configuration"
                )
            ...
            for proxy_var in ["HTTP_PROXY", "HTTPS_PROXY"]:
                ...
                if proxy_url and not (
                    proxy_url.startswith("http://") or proxy_url.startswith("https://")
                ):
                    results["issues"].append(
                        f"{proxy_var} should be a valid URL starting with http:// or https://"
                    )
            ...
            if no_proxy:
                ...
                if missing_exclusions:
                    results["recommendations"].append(
                        f"Consider adding these to NO_PROXY: {', '.join(missing_exclusions)}"
                    )
            ...
            if proxy_secret_arn:
                ...
                if not proxy_secret_arn.startswith("arn:aws:secretsmanager:"):
                    results["issues"].append(
                        "PROXY_SECRET_ARN should be a valid AWS Secrets Manager ARN"
                    )
                    results["recommendations"].append(
                        "Fix PROXY_SECRET_ARN format - should start with arn:aws:secretsmanager:"
                    )
                        ...
                        try:
                            ...
                            try:
                                ...
                                if "SecretString" in secret_value:
                                    ...
                                    if missing_fields:
                                        results["issues"].append(
                                            f"Proxy secret is missing required fields: {', '.join(missing_fields)}"
                                        )
                                        results["recommendations"].append(
                                            f"Update the secret at {proxy_secret_arn} to include all required fields"
                                        )
                                    else:
                                        results["vpc_config"][
                                            "proxy_secret_valid"
                                ...
                                results["recommendations"].append(
                                    f"Couldn't validate proxy secret structure: {str(e)}"
                                )

                        except secretsmanager.exceptions.ResourceNotFoundException:
                            results["issues"].append(
                                f"Proxy secret {proxy_secret_arn} does not exist"
                            )
                            results["recommendations"].append(
                                f"Create the secret at {proxy_secret_arn} with required fields"
                            )
                            results["valid"] = False

                except Exception as e:
                    results["recommendations"].append(
                        f"Couldn't check proxy secret existence: {str(e)}"
                    )
        ...
        if vpc_in_use:
            if not vpc_provider_configured:
                results["recommendations"].append(
                    "VPC configuration found but VPC resource provider is not explicitly configured"
                )

            # Check if VPC ID is provided when needed
            if (
                not vpc_config.get("vpcId")
                and not vpc_env_vars["VPC_ID"]
                and vpc_from_existing
            ):
                results["issues"].append(
                    "Using existing VPC but VPC ID is not specified"
                )
                results["recommendations"].append(
                    "Specify VPC ID either in code or via VPC_ID environment variable"
                )

            # Check if subnets are specified
            if vpc_from_existing and not vpc_config.get("subnets"):
                results["recommendations"].append(
                    "Using existing VPC but subnet IDs are not specified in code"
                )

            # Check if security groups are specified
            if vpc_from_existing and not vpc_config.get("securityGroups"):
                results["recommendations"].append(
                    "Using existing VPC but security group IDs are not specified in code"
                )

        # General recommendations for VPC setup
        if vpc_in_use and not vpc_proxy_configured:
            results["recommendations"].append(
                "VPC is configured but no proxy settings found. If internet access is needed, configure proxy or NAT Gateway"
            )

        if vpc_proxy_configured and not vpc_in_use:
            results["issues"].append(
                "Proxy configuration found but VPC is not properly configured"
            )
            results["recommendations"].append(
                "Ensure VPC resource provider is configured when using proxy"
            )
