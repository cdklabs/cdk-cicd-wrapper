...
# Requires docker and the locally packed zero-touch tarballs (development/v3-tgz/*.tgz -- `npm pack` per package).
# =============================================================================
set -uo pipefail
...
main_deploy() {
  ...
  [ -f "$wrapper_tgz" ] && [ -f "$cli_tgz" ] || die "missing zero-touch tarballs in $tgz_dir (run: npm pack per package)"

  local bundle rc=0
  bundle="$(mktemp -d)"

  # --- bundle: level1 fixture + vendored deps (wrapper CLI from the local tgz) ----------------------
  log 'leg 1: assemble the deployer bundle (level1 fixture + vendored zero-touch deps)'
