#!/usr/bin/env bash
# =============================================================================
# global-ddb-verify -- proves Autopilot deploys a DynamoDB GLOBAL TABLE across regions. From the global-ddb-app
...
main_ddb() {
  ...
  # --- deploy via the Autopilot CLI (single stage; the replica is created cross-region) --------------------
  ...
  [ "$rc" = 0 ] && log 'global-ddb-verify PASSED: Autopilot deployed a DynamoDB global table across two regions, then torn down' \
